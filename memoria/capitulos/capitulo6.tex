\chapter{Plataforma software del sistema}
\label{cap:capitulo6}
...
Una vez definida la arquitectura \textit{hardware} del sistema desarrollado, en este capítulo se presenta la plataforma \textit{software} encargada de dar soporte al robot.

\vspace{0.2cm}

El \textit{software} se ha diseñado de forma modular con el objetivo de facilitar el desarrollo, mantenimiento, depuración y escalabilidad del sistema. Como se muestra en la Figura \ref{fig:sistema}, el robot se compone de distintos módulos funcionales que serán descritos a continuación.
...
Debido a las limitaciones computacionales de la Raspberry Pi 4, no es posible ejecutar tareas de alto coste computacional. Por ese motivo, se ha diseñado una arquitectura distribuida en la que las tareas más exigentes se delegan a un servidor local.
...
\begin{figure} [h!]
  \begin{center}
    \includegraphics[width=16cm]{figs/server}
  \end{center}
  \caption{Servidor local del sistema}
  \label{fig:server}
\end{figure}\
...
Para asegurar una comunicación estable entre la Raspberry Pi y el servidor, se ha utilizado un router propio configurado con una red local privada. De este modo, ambos dispositivos poseen direcciones IP fijas, evitando problemas de cambios de direcciones IP.


\section{Sistema de autenticación facial}
\label{sec:Sistema de percepción}

El reconocimiento facial es el primer paso para arrancar el sistema. Este módulo se apoya en la arquitectura distribuida previamente descrita. El sistema se divide en dos fases principales: registro de usuarios e inicio de sesión.
...
La arquitectura del sistema conversacional está compuesta por una serie de etapas consecutivas: captura de audio, transcripción, procesamiento mediante un modelo de lenguaje natural, y conversión del texto generado a voz.

El sistema permanece en un estado de escucha continua y únicamente se activa cuando se detecta una palabra de activación predefinida. Este mecanismo evita generar respuestas no deseadas, así como agotar los recursos del sistema.
...
La interfaz se ha implementado utilizando la biblioteca \verb|PyQt5| y se ejecuta en la Raspberry Pi. El diseño sigue una arquitectura basada en estados, donde cada pantalla representa una funcionalidad distinta del sistema. La transición entre pantallas se realiza mediante eventos gestionados por un controlador central.
...
Durante el diseño de la interfaz se ha priorizado la accesibilidad, teniendo en cuenta que el sistema está orientado a personas mayores. Para ello se ha optado por la utilización de botones de gran tamaño, iconografía fácilmente reconocible, y una navegación sencilla.
...
\begin{figure} [h!]
  \begin{center}
    \includegraphics[width=9cm]{figs/login}
  \end{center}
...
El comportamiento de este sistema se basa en eventos programados. Cuando se detecta que ha trascurrido un período de tiempo definido, el sistema genera una sugerencia de forma aleatoria, la cual se presenta en la interfaz gráfica y mediante síntesis de voz. En la siguiente Figura \ref{fig:sugerencia} se muestra un ejemplo de las sugerencias generadas por este módulo.
...
La aplicación de calendario (Figura \ref{fig:calen}) permite gestionar de forma visual los eventos del usuario garantizando la persistencia de los datos entre sesiones.
...
La interacción con esta aplicación permite la creación, eliminación y visualización de eventos de forma intuitiva. Por otro lado, el sistema de recordatorios permite al usuario gestionar avisos pendientes. Esta funcionalidad (Figura \ref{fig:rec}) sigue una estructura similar a la aplicación de calendario, compartiendo modelo de persistencia y visualización.
...
Estos juegos siguen el mismo enfoque sencillo que el resto de aplicaciones del sistema, con elementos visuales de gran tamaño y una interacción intuitiva. En la Figura \ref{fig:mem} se muestra el juego de memoria desarrollado.
