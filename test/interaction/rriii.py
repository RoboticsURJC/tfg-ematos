...
EXPRESIONES = {
    ...
    "hablando":  {"boca": "sonrisa", "cejas": "plano"},
}
...
def dibujar_cara(expresion="feliz", ojos_abiertos=True):
    ...
    elif estado["boca"] == "sorpresa" or expresion=="hablando":
...
def hablar_piper(texto, modelo="/home/eli/tfg-ematos/test/interaction/es_ES-sharvard-medium.onnx"):
    """Genera voz con Piper y reproduce el audio"""
    poner_cara("hablando", duracion=len(texto)*0.05 + 0.5)
    archivo = "/tmp/tts_output.wav"
    cmd = ["piper", "--model", modelo, "--output_file", archivo, texto]
    subprocess.run(cmd, check=True)
    subprocess.run(["aplay", archivo], check=True)
...
def revisar_recordatorios():
    while True:
        ...
        for r in recordatorios[:]:
            if ahora >= r["time"]:
                hablar_piper(f"⏰ Recordatorio: {r['text']}")
...
def procesar_texto(texto):
    ...
    if "hora" in texto:
        ...
        try:
            ...
            hablar_piper(f"Vale, te recordaré en {minutos} minutos")
        except:
            hablar_piper("No entendí el tiempo")
            poner_cara("triste", 2)
...
model_vosk = vosk.Model("/home/eli/tfg-ematos/test/voice/vosk-model-small-es-0.42")
...
with sd.RawInputStream(
    ...
    print("🤖 Robot activo: escucha, habla y reacciona")
    while True:
        ahora = time.time()
