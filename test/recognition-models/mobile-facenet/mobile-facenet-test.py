...
try:
    ...
    print("Edge TPU detected and working")
except Exception as e:
    print("Error at charging Edge TPU:", e)
    detector = tflite.Interpreter(model_path="face_detection_front.tflite")
    recognizer = tflite.Interpreter(model_path="face_detection_front.tflite")
...
while True:
    ...
    for i in range(len(confidences)):
        if confidences[i] > 0.75:
            ...
            label = f"{person_name} ({score:.2f})" if person_name else "Desconocido"
